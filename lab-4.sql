...
SELECT count(DISTINCT customer.CUS_ID) AS total, customer.CUS_GENDER FROM customer
INNER JOIN 
...
SELECT MIN(supplier_pricing.SUPP_PRICE) as ProductPrice, category.CAT_ID As CategoryID, category.CAT_NAME AS CategoryName
FROM 
product
INNER JOIN 
...
SELECT product.PRO_ID AS Id, product.PRO_NAME AS Name FROM
`order`
INNER JOIN
supplier_pricing ON supplier_pricing.PRICING_ID = `order`.PRICING_ID
INNER JOIN
...
WHEN avg( RAT_RATSTARS ) > 4 THEN 'Average Service'
ELSE 'Poor Service'
